...
private:
    void debug()
    {
        ATOMIC(std::cout << BLUE_MSG << "MemPool: size " << SIZE << " (" << sizeof(*this) << ") = #elements " << (SIZEOF(m_storage) - 1) << "+1, sizeof(storage elements) " << sizeof(m_storage[0]) << ENDCOLOR << std::flush);
    }
...
//class ShmVar: std::shared_ptr<TYPE> //use smart ptr to clean up afterward
//{
//public: //ctor/dtor
...
//    PERFECT_FWD2BASE_CTOR(ShmVar, INNER_CREATE) {}
//#undef INNER_CREATE
//public: //operators
...
//    PERFECT_FWD2BASE_CTOR(ShmVar, INNER_CREATE), m_clup(&m_var, shmdeleter<TYPE>()) {}
//#undef INNER_CREATE
//public: //operators
...
{
    ATOMIC(std::cout << PINK_MSG << "data space:" <<ENDCOLOR << std::flush);
    void* ptr1 = pool20->alloc(10);
    void* ptr2 = pool20->alloc(4);
    pool20->free(ptr2);
    void* ptr3 = pool20->alloc(1);

    ATOMIC(std::cout << PINK_MSG << "stack:" <<ENDCOLOR << std::flush);
    ...
    ATOMIC(std::cout << PINK_MSG << "shmem: actual size " << sizeof(pool_type) << ENDCOLOR << std::flush);
...
//    SHM_DECL(pool_type, shmpool); //equiv to "pool_type shmpool", but allocates in shmem
    ShmVar<pool_type> shmpool; //put it in shared memory
