...
//allocate memory:
void* shmalloc(size_t size, key_t key = 0)
{
    ...
    ATOMIC(std::cout << CYAN_MSG << "shmalloc: cre shmget key " << FMT("0x%lx") << key << ", size " << size << " => " << FMT("id 0x%lx") << shmid << ENDCOLOR << std::flush);
...
//    if ((shmid == -1) && (errno == ENOENT)) return; //didn't exist
    ...
    ATOMIC(std::cout << CYAN_MSG << "shmfree: freed " << FMT("key 0x%lx") << info.key << FMT(", id 0x%lx") << info.id << ", size " << info.size << ENDCOLOR << std::flush);
}
