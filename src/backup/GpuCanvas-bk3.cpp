...
//assign dimming slots to each row
    ...
    {
//    int count = rcinx - ROW(rcinx) - HASDIAG(rcinx); //skip diagonal ch*plex row/col address
        printf(BLUE_LT "resolve dups rc %d, # %d" ENDCOLOR, rcinx, count);
...
//        DispList[disp_count].delay = 0;
        ...
        for (int i = 0; i < disp_count; i += 3)
            printf("disp[%d/%d]: delay %d (dim %d), rowmap 0x%x (row %d), colmap 0x%x\n", i, disp_count, DispList[i]/*.delay*/, dim -= DispList[i]/*.delay*/, DispList[i + 1]/*.rowmap*/, Log2(DispList[i + 1]/*.rowmap*/), DispList[i + 2]/*.colmap*/);
    }
//helpers:
private:
...
#define UTYPEOF(univtype)  ((univtype) & TYPEBITS)
    ...
    UniverseTypes UnivType(int inx, UniverseTypes newtype = INVALID)
    {
        ...
        if (UTYPEOF(newtype) != UTYPEOF(oldtype))
        {
...
//                                    myprintf(14, BLUE_LT "pixel[%d] 0x%x -> br %d" ENDCOLOR, xofs + yy, pixels? pixels[xofs + yy]: fill, brightness);
...
//                            uint8_t byte_odd = (ctlr_ofs < 0)? this->encoders[x].cast->checksum: (this->encoders[x].cast->DispList[2 * ctlr_ofs + 1];
                            ...
                            uint8_t byte_odd = !ctlr_ofs? this->encoders[x].cast->checksum: this->encoders[x].cast->DispList[2 * ctlr_ofs - 1];
