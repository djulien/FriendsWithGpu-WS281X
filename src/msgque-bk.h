...
class Signal
{
    ...
    std::unique_lock</*VOLATILE std::mutex*/ decltype(m_mutex)>* m_ulk; //needed for wait on cond var
//        typedef /*typename*/ /*std::decay<decltype(m_ulk)>::type*/ std::decay<decltype(*m_ulk)> lock_type;
public: //ctor/dtor
    Signal(): m_ulk(0) {}
    ~Signal() { if (islocked()) unlock(); }
public: //members
    inline bool islocked() const { return m_ulk; }
    void lock()
    {
        if (islocked()) throw "already locked";
//            m_ulk = new lock_type(m_mutex); //std::unique_lock<VOLATILE std::mutex>(ptr->m_mutex);
//no worky; compiler bug?            m_ulk = new std::decay<decltype(*m_ulk)>(m_mutex);
        m_ulk = new std::unique_lock<VOLATILE std::mutex>(m_mutex);
    }
    void wait(int poll = 0)
    {
        if (!poll || islocked()) m_condvar.wait(*m_ulk); //scoped_lock);
...
{
    sleep_msec(500);
    ATOMIC_MSG(PINK_MSG << timestamp() << "child start, thrid " << thrid() << ", thrinx " << testq->thrinx() << ", send now" << ENDCOLOR);
...
//    int thrinx = testq->thrinx();
    ATOMIC_MSG(PINK_MSG << timestamp() << "main start, thrid " << thrid() << ", thrinx " << testq->thrinx() << ENDCOLOR);
