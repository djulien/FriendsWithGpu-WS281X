...
//minimal memory pool:
//NOTE: does not maintain lists; only reclaims space at end
...
#if 1
WithMutex<MemPool<rdup(10, 8)+8 + rdup(4, 8)+8>> pool20; //data seg
int main(int argc, const char* argv[])
{
    ATOMIC(std::cout << PINK_MSG << "data seg:" << ENDCOLOR);
    ...
    MemPool<rdup(1, 8)+8 + rdup(1, 8)+8> pool10; //stack (doesn't need mutex; can't be shared)
    void* ptr4 = pool10.alloc(1);
    void* ptr5 = pool10.alloc(1);
    void* ptr6 = pool10.alloc(0);

    ATOMIC(std::cout << PINK_MSG << "shmem:" << ENDCOLOR);
#define MPSIZE  rdup(10, 8)+8 + rdup(4, 8)+8
    typedef WithMutex<MemPool<MPSIZE>> pool_type;
//    pool_type shmpool20; //shmem
    pool_type* shmpool = new (static_cast<pool_type*>(shmalloc(MPSIZE))) pool_type();
//    std::shared_ptr<pool_type> shmpool(static_cast<pool_type*>(shmalloc(MPSIZE)), shmdeleter<pool_type>()); //[](auto ptr) { shmfree(ptr); }); //see lambda deleter at: http://en.cppreference.com/w/cpp/memory/shared_ptr/shared_ptr
...
{  
    ...
    TYPE* allocate(const size_t count = 1) const
    {
...
//        pointer ptr = static_cast<pointer>(::operator new(count * sizeof(type), ::std::nothrow));
        ...
        m_used += count;
        ATOMIC(std::cout << YELLOW_MSG << "ShmHeap: allocated " << count << " bytes at " << FMT("%p") << ptr << ", " << avail() << " bytes remaining" << ENDCOLOR << std::flush);
        return ptr;
    }
