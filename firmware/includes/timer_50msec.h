...
//with 8:1 prescalar, Timer 1 interval is 1 usec @ 8 MIPS, preset for 50 msec tick == 50,000 == 0x3cb0
    ...
    volatile AT_NONBANKED(0) uint32_t tmr1_limit_debug; //= Timer1_Limit;
...
//tmr1_ticks_test2_debug = Instr2uSec(8 * Timer1_Prescalar, CLOCK_FREQ); //max duration for Timer 1; avoid arith overflow error in BoostC by dividing by 2
    t1con_debug = MY_T1CON(CLOCK_FREQ / PLL);
    tmr1_preset_debug = TMR1_PRESET_50msec; //should be ~ -50000 == 0x3cb0 with 8:1 pre @ 8 MIPS
    tmr1_50msec_debug = 50 msec;
    tmr1_presbits_debug = NumBits8(Timer1_Prescalar); //should be 4 (for prescalar 8)
    tmr1_prescalar_debug = Timer1_Prescalar; //should be 8 @ 8 MIPS, makes T1 tick == 1 usec
    tmr1_limit_debug = U16FIXUP(Timer1_Range);
    tmr1_limit_debug = Timer1_Limit; //65.5 msec @ 8 MIPS with prescalar 8
//#define Timer1_Limit  Instr2uSec(256 * 256 * Timer1_Prescalar, CLOCK_FREQ) //max duration for Timer 1; avoid arith overflow error in BoostC by dividing by 2
    tmr1_limit_debug = Instr2uSec(256 * 256 * Timer1_Prescalar, CLOCK_FREQ); //BROKEN; max duration for Timer 1; avoid arith overflow error in BoostC by dividing by 2
    tmr1_limit_debug = Instr2uSec(0x10000 * Timer1_Prescalar, CLOCK_FREQ); //max duration for Timer 1; avoid arith overflow error in BoostC by dividing by 2
...
//    tmr1_8thlimit_debug = Timer1_8thLimit;
    tmr1_ticks_test1_debug = TMR1_TICKS(ONE_SEC);  //should be 20
    tmr1_ticks_test2_debug = TMR1_TICKS(280 msec); //BROKEN; should be ~ 5
 }
 #undef debug
...
//    TMR1L += TMR1_PRESET_50msec % 0x100;
    WREG = TMR1_PRESET_50msec % 0x100; addwf(ASMREG(TMR1L));
    WREG = TMR1_PRESET_50msec / 0x100; addwfc(ASMREG(TMR1H));
//    TMR1L += TimerPreset(50 msec / 2, 8, Timer1, CLOCK_FREQ) % 0x100; // / 0x100; /*BoostC sets LSB first, which might wrap while setting MSB; explicitly set LSB first here to avoid premature wrap*/
//    if (CARRY) ++TMR1H;
//    TMR1H += TimerPreset(50 msec / 2, 8, Timer1, CLOCK_FREQ) / 0x100; /*BoostC sets LSB first, which might wrap while setting MSB; explicitly set LSB first here to avoid premature wrap*/
