...
//use remaining 2 texture lookups to retrieve other parts of caller's original pixel color:
    ...
    float which = x - float(inx); //[0, 4, 8, 12, 16, 20]
...
//        if (nabe0.a == 0.0) //transparency; let GPU fx show through
//TODO: use gpufx().a for blending?
    vColor = gpufx(x, part0[inx]);
#else
//get remaining parts and reconstruct caller's original pixel color:
    vec4 part1 = SAMPLE(which + mod(byte + 1.0, 3.0), y);
    vec4 part2 = SAMPLE(which + mod(byte + 2.0, 3.0), y);
//assumes A was 1.0 (not enough texture lookups left on RPi to get that)
    vColor = IF(byte == 0.0, vec4(part0[inx], part1[inx], part2[inx], 1.0)) + IF(byte == 1.0, vec4(part2[inx], part0[inx], part1[inx], 1.0)) + IF(byte == 2.0, vec4(part1[inx], part2[inx], part0[inx], 1.0));
#endif //def CUSTOM_GPUFX
#ifdef WS281X_DEBUG
    if ((which != 0.0) && (which != 4.0) && (which != 8.0) && (which != 12.0) && (which != 16.0) && (which != 20.0)) vColor = ERROR(1);
    if ((byte != 0.0) && (byte != 1.0) && (byte != 2.0)) vColor = ERROR(1);
